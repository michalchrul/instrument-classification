from config import datasets_path, convoluted_path,
import os
import librosa
...
trainset_path = datasets_path / 'nsynth-train' / 'audio'
testset_path = datasets_path / 'nsynth-test' / 'audio'
validset_path = datasets_path / 'nsynth-valid' / 'audio'
...
def mixdown_dataset(dataset_path, output_path):
    ...
    for filename in tqdm(list_of_files):
        ...
        for i in range(1, max):
            ...
            sample_B, sr = librosa.load(random_path, mono=True, sr=16000, duration=4)
...
sample_A = convoluted_path / 'mix_check' / 'bass_electronic_027-033-127_14.wav'
sample_B = convoluted_path / 'mix_check' / 'keyboard_electronic_003-045-075_15.wav'
sample_C = convoluted_path / 'mix_check' / 'vocal_acoustic_000-057-127_423.wav'

#mix_samples_test(sample_A, sample_B, sample_C)

#convolve_dataset(testset_path, output_test_path)
#convolve_dataset(validset_path, output_valid_path)
#convolve_dataset(trainset_path, output_train_path)


mixdown_dataset(conv_valid_path, mix_valid_path)
mixdown_dataset(conv_train_path, mix_train_path)

#mixdown_dataset(output_train_path, convoluted_path)
